...
from simproj.apps.indicadores.models import SELIC, IGPM, IPCA
        ...
        with open(directory+"/tesouro.csv", "rU") as csvfile:
            ...
            for line in r:
                ...
                try:
                    ...
                    maturity = datetime.datetime.strptime(line[2], "%d/%m/%Y")
                    obj = TesouroDireto.objects.get(name=line[0], maturity=maturity)

                except TesouroDireto.DoesNotExist:

                    obj = TesouroDireto(name=line[0], maturity=maturity)

                obj.description = line[1]
                obj.rate = float(line[3])
