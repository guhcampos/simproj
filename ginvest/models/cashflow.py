...
from simproj.apps.indicadores.models import SELIC, IPCA, IGPM
...
class CashFlowSeries(models.Model):
    ...
    def __unicode__(self):
        return "CashFlowSeries: IPCA: {ipca}, IGP-M: {igpm}, SELIC: {selic}".format(
...
class CashFlow(models.Model):
    ...
    def __unicode__(self):
         return "CashFlow '{name}', due in {date}. Value: R$ {value:6.2f}, NPV: R$ {npv:6.2f}".format(
             name=self.description, date=self.date, value=self.value, npv=self.npv
             )
